...
public class OSSStorageEngine extends AbstractStorageEngine {
     ...
     *
     * OSS文件分片上传的步骤：
     * 1、初始化文件分片上传，获取全局唯一的uploadId
     * 2、并发上传文件分片，每一个文件分片都需要带有初始化返回的uploadId
     * 3、所有分片上传完成，触发文件分片合并的操作
     *
     * 难点：
     * 1、分片上传是在一个多线程并发的环境下运行的，我们程序需要保证我们的初始化分片上传的操作只有一个线程可以做
     * 2、所有的文件分片都需要带有一个全局唯一的uploadId，该uploadId就需要放到一个线程的共享空间中
     * 3、需要保证每一个文件分片能够单独的去调用文件分片上传，而不是依赖于全局的uploadId
     *
     * 解决方案：
     * 1、加锁：目前基于单体架构的考虑，使用JVM的锁去保证一个线程初始化文件分片上传。后续扩展成分布式的架构，需要更换分布式锁
     * 2、使用缓存：缓存分为本地缓存（redis）和分布式缓存。虽然目前是单体架构，但是仍然直接使用分布式缓存
     * 3、我们想要把每一个文件的Key都能够通过文件的url来获取，就需要定义一种数据格式，支持我们添加附件数据，并且可以很方便的解析出来，
     *解析方案可以参考网络请求的URL格式：fileRealPath?paramKey=paramValue
     *
     * 具体的实现逻辑如下：
     * 1、检验文件分片数不得大于10000
     * 2、获取缓存key
     * 3、通过缓存key获取初始化后的对象，获取全局的uploadId和ObjectName
     * 4、如果获取为空，直接初始化
     * 5、执行文件分片上传你的操作
     * 6、上传完成之后，将全局的参数封装成一个可识别的url，保存在上下文里面，用于业务的落库操作
     *
     * @param context
     */
    ...
    protected void doStoreChunk(StoreFileChunkContext context) throws IOException {
        if (context.getTotalChunks() > TEN_THOUSAND_INT) {
            throw new RPanFrameworkException("分片数超过了最大上限，分片数不得大于 " + TEN_THOUSAND_INT);
        }
    ...
    public static class ChunkUploadEntity implements Serializable {
        ...
        private static final long serialVersionUID = -2999467841966366395L;
