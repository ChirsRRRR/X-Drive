...
public class FileController {
    ...
    public R<UploadedChunksVO> getUploadedChunks (@Validated QueryUploadedChunksPO queryUploadedChunksPO) {
        QueryUploadedChunksContext context = fileConverter.queryUploadedChunksPO2QueryUploadedChunksPOContext(queryUploadedChunksPO);
    ...
    public R<List<BreadcrumbsVO>> getBreadcrumbs(@NotBlank(message = "文件ID不能为空") @RequestParam(value = "fileId", required = false) String fileId) {
        ...
        context.setFileId(IdUtil.decrypt(fileId));
        context.setUserId(UserIdUtil.get());
        List<BreadcrumbsVO> result = iUserFileService.getBreadcrumbs(context);
        return R.data(result);
    }
