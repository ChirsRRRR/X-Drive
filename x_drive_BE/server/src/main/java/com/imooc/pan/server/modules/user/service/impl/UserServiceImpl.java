...
public class UserServiceImpl extends ServiceImpl<RPanUserMapper, RPanUser> implements IUserService {
     ...
     *
     * 实现技术难点的处理方案
     * 1、幂等性通过数据库表对用户名字段添加唯一索引来实现，上游业务捕获对应的冲突异常，转化返回给前端
     * @param userRegisterContext
     * @return
     */
    @Override
    public Long register(UserRegisterContext userRegisterContext) {
        assenbleUserEntity(userRegisterContext);
    ...
    public String login(UserLoginContext userLoginContext) {
        ...
        generateAndSaveAcccessToken(userLoginContext);
    ...
    public UserInfoVO info(Long userId) {
        ...
        if (Objects.isNull(entity)) {
            throw new RPanBusinessException("用户不存在，用户信息查询失败");
        }

        RPanUserFile userRootFolder = getUserRootFileInfo(userId);
        if (Objects.isNull(userRootFolder)) {
            throw new RPanBusinessException("用户根文件夹信息不存在，用户信息查询失败");
        }

        return userConverter.assembleUserInfoVO(entity, userRootFolder);
    }
...
//        return super.listByIds(idList);
    ...
    /**
     * 实体转化
     * 由上下文信息转化为用户实体 封装进上下文
     * @param userRegisterContext
     */
    private void assenbleUserEntity(UserRegisterContext userRegisterContext) {
     ...
     * @param userLoginContext
     */
    private void generateAndSaveAcccessToken(UserLoginContext userLoginContext) {
    ...
    private void checkLoginInfo(UserLoginContext userLoginContext) {
        ...
        if (!Objects.equals(encPassword, dbPassword)) {
            throw new RPanBusinessException("密码错误，请输入正确密码！");
        }
    ...
    private void checkForgetPasswordToken(ResetPasswordContext resetPasswordContext) {
        ...
        if (!Objects.equals(tokenUsername, resetPasswordContext.getUsername())) {
            throw new RPanBusinessException("token无效");
        }
    }
