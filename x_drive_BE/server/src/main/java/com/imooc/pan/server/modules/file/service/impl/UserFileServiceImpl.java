...
public class UserFileServiceImpl extends ServiceImpl<RPanUserFileMapper, RPanUserFile>  implements IUserFileService {
    ...
    public List<RPanUserFileVO> getFileList(QueryFileListContext context) {
        return baseMapper.getFileList(context);
    }
     ...
     * @param context
     */
    // @Transactional 表示saveFile()方法和saveUserFile()方法，要不然都成功或者都失败
    ...
    public UploadedChunksVO getUploadedChunks(QueryUploadedChunksContext context) {
        ...
        List uploadedChunks = iFileChunkService.listObjs(queryWrapper, value -> (Integer) value);
    ...
    public List<BreadcrumbsVO> getBreadcrumbs(QueryBreadcrumbsContext context) {
        List<RPanUserFile> folderRecords = queryFolderRecords(context.getUserId());
        Map<Long, BreadcrumbsVO> prepareBreadcrumbVOMap = folderRecords.stream().map(BreadcrumbsVO::transfer).collect(Collectors.toMap(BreadcrumbsVO::getId, a -> a));
    ...
    public List<RPanUserFile> findAllFileRecords(List<RPanUserFile> records) {
        ...
        if (folderCount == RPanConstants.ZERO_LONG) {
            return result;
        }
    ...
    private void assembleCopyChildRecord(List<RPanUserFile> allRecords, RPanUserFile record, Long targetParentId, Long userId) {
        ...
        handleDuplicatedFilename(record);
    ...
    private void checkCopyCondition(CopyFileContext context) {
        ...
        if (checkIsChildFolder(prepareRecords, targetParentId, context.getUserId())) {
            throw new RPanBusinessException("目标文件夹ID不能少选中文件列表的文件夹ID或者其子文件夹的ID");
        }
    }
    ...
    private boolean checkIsChildFolder(List<RPanUserFile> prepareRecords, Long targetParentId, Long userId) {
        ...
        List<Long> unavailableFolderRecordsIds = unavailableFolderRecords.stream().map(RPanUserFile::getFileId).collect(Collectors.toList());
    ...
    private List<FolderTreeNodeVO> assembleFolderTreeNodeVOList(List<RPanUserFile> folderRecords) {
        ...
        //一直都是对象引用的copy操作，而没有在堆上进行重新创建的操作
        // 其中的分组操作(group by) 不会改变对象的原有引用
     ...
     *
     * 1、文件记录必须存在
     * 2、文件记录的创建者必须属于当前登录的用户
     *
     * @param record
     * @param userId
     */
    private void checkOperationPermission(RPanUserFile record, Long userId) {
        if (Objects.isNull(record)) {
            throw new RPanBusinessException("当前文件记录不存在");
        }
    ...
    private Long saveUserFile(Long parentId,
                              ...
                              Long userId,
                              String fileSizeDesc) {
        RPanUserFile entity =  assembleRPanUserFile(parentId, filename, folderFlagEnum, fileType, realField, userId, fileSizeDesc);
        if (!save(entity)) {
     ...
     * @return
     */
    private RPanUserFile assembleRPanUserFile(Long parentId, String filename, FolderFlagEnum folderFlagEnum, Integer fileType, Long realField, Long userId, String fileSizeDesc) {
        ...
        entity.setRealFileId(realField);
        ...
        handleDuplicatedFilename(entity);
    ...
    /**
     * 处理用户重复明明
     * 如果同一文件夹下有文件名重复，则按照系统规则重命名文件夹
     *
     * @param entity
     */
    private void handleDuplicatedFilename(RPanUserFile entity) {
    ...
    private void checkUpdateFilenameCondition(UpdateFilenameContext context) {
        ...
        if (Objects.equals(entity.getFilename(), context.getUserId())) {
            throw new RPanBusinessException("请输入新的文件名来修改");
        }
