...
public class ShareServiceImpl extends ServiceImpl<RPanShareMapper, RPanShare>
    ...
    public List<RPanUserFileVO> fileList(QueryChildFileListContext context) {
        RPanShare record = checkShareStatus(context.getShareId());
        context.setRecord(record);
        List<RPanUserFileVO> allUserFileRecords = checkFileIsOnShareStatusAndGetAllShareUserFiles(context.getShareId(), Lists.newArrayList(context.getParentId()));
    ...
    public void saveFiles(ShareSaveContext context) {
        ...
        checkFileIsOnShareStatus(context.getShareId(), context.getFileIdList());
        doSaveFiles(context);
    }
    ...
    public void download(ShareFileDownloadContext context) {
        checkShareStatus(context.getShareId());
        checkFileIsOnShareStatus(context.getShareId(), Lists.newArrayList(context.getFileId()));
        doDownload(context);
    }
     ...
     *
     * 1、查询所有受影响的分享的ID集合
     * 2、去判断断每一个分享对应的文件以及所有的父文件信息是否均为正常，该种情况，把分享的状态变为 正常
     * 3、如果有分享的文件或者是父文件被删除，变更该分享的状态为有文件被删除
     *
...
//        return super.listByIds(idList);
     ...
     *
     * 1、查询对应的分享信息
     * 2、去判断分享对应的文件以及所有的父文件信息均为正常，该种情况，被分享的状态变为正常
     * 3、如果有分享的文件或是父文件信息被删除，变更该分享的状态为有文件被删除
     * @param shareId
     */
    ...
    private void doChangeShareStatus(RPanShare record, ShareStatusEnum shareStatus) {
        ...
        if (!updateById(record)) {
            producer.sendMessage(PanChannels.ERROR_LOG_OUTPUT, new ErrorLogEvent( "更新分享状态失败，请手动更改状态，分享ID为: " +
                    record.getShareId() + ", 分享状态改为: " + shareStatus.getCode(), RPanConstants.ZERO_LONG));
        }
    }
     ...
     * @param fileIdList
     */
    private void checkFileIsOnShareStatus(Long shareId, List<Long> fileIdList) {
        checkFileIsOnShareStatusAndGetAllShareUserFiles(shareId, fileIdList);
    }
     ...
     * @return
     */
    private List<RPanUserFileVO> checkFileIsOnShareStatusAndGetAllShareUserFiles(Long shareId, List<Long> fileIdList) {
        List<Long> shareFileIdList = getShareFileIdList(shareId);
    ...
    private String generateShareToken(CheckShareCodeContext context) {
        ...
        String token = JwtUtil.generateToken(UUIDUtil.getUUID(), ShareConstants.SHARE_ID, record.getShareId(), ShareConstants.ONR_HOUR_LONG);
        return token;
    }
