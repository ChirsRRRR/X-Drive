...
public class RecycleServiceImpl implements IRecycleService {
     ...
     * 检查要还原的文件名称是否被占用
     *
     * 1、要还原的文件列表中同一文件夹下面相同名称的文件，不能还原
     * 2、要还原的文件在当前的父文件夹下存在同名文件，不能还原
     *
     * @param context
     */
    private void checkRestoreFilename(RestoreContext context) {
        ...
        if (filenameSet.size() != records.size()) {
            throw new RPanBusinessException("文件还原失败，该还原文件中存在同名文件，请逐个还原并重新命名");
        }
        ...
        for (RPanUserFile record : records) {
            ...
            if (iUserFileService.count(queryWrapper) > 0) {
                throw new RPanBusinessException("文件: " + record.getFilename() + " 还原失败，该文件夹下已存在相同名称的文件夹或者文件，请重新命名之后再执行文件还原的操作！");            }
        }
    }
